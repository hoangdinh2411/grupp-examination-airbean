...
export default function Status() {
  ...
  function getETA(orderNR) {
    fetching(`beans/order/status/${orderNR}`, 'GET')
      .then((res) => {
        console.log(res)
        setEta(res.eta)
      })
  ...
  return (
    <div className='status-wrapper'>
      <section className='status-wrapper__orderNr'>
        {' '}
        Ordernummer <b>#{orderNR}</b>{' '}
      </section>
      <img className='status-wrapper__img' src={drone} alt='drone' />
      <h1 className='status-wrapper__h1'> Din beställning är på väg!</h1>
      <section className='status-wrapper__eta'>
        {' '}
        <b>{eta}</b> minuter{' '}
      </section>
      <Link to='/menu'>
        <Button variant='light'> Ok, cool! </Button>
      </Link>
    </div>
  )
}
